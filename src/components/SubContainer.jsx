...
export const SubContainer = ({ subcontainerId }) => {
  ...
  const thisSubContainer = elements[subcontainerId];
  ...
  const handleChildClick = (e) => {
    ...
    if(edit){
      ...
      if (thisSubContainer.dir !== null) {
        updateData((updater) => {
          updater.whichpage = thisSubContainer.dir;
        });
      }else if(thisSubContainer.statedir !== null){
        updateData((updater) => {
          updater.elements[thisSubContainer].whichstate = thisSubContainer.statedir;
        });
      }
    }
  };
  ...
  for (let i = 0; i < subContainerChildArr.length; i++) {
    ComponentsArray.push(
      CompSwitcher(elements[subContainerChildArr[i]].name, subContainerChildArr[i])
    );
  }
